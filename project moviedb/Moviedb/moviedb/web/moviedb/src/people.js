...
export default function Movie() {
  ...
  useEffect(() => {
    ...
    fetch("https://moviedb.herokuapp.com/people/"+query).then(resp => resp.json()).then((data) => setPeople(data)).catch(() => {window.location.href="/"});
  },[])

  return (
  	<div>
  	<Helmet>
    <title>{ people?(people.name+" | moviedb"):"moviedb - Movies Reviewed by people, for people" }</title>
    </Helmet>
