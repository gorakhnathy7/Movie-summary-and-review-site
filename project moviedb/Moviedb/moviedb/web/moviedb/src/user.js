...
export default function User() {
  ...
  useEffect(() => {
    ...
    fetch("https://moviedb.herokuapp.com/user/"+id).then(resp => resp.json()).then((data) => setProfile(data)).catch(console.log);
  },[])

  return (
  	<div>
    <Helmet>
    <title>{ profile?("User "+profile.user_id):"moviedb - Movies Reviewed by people, for people" }</title>
    </Helmet>
