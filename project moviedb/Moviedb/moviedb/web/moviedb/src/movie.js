...
export default function Movie() {
  ...
  useEffect(() => {
    ...
    fetch("https://moviedb.herokuapp.com/movie/"+query).then(resp => resp.json()).then((data) => setMovie(data)).catch(() => {window.location.href="/"});
  },[])

  return (
  	<div>
  	<Helmet>
    <title>{ movie?(movie.title+" | moviedb"):"moviedb - Movies Reviewed by people, for people" }</title>
